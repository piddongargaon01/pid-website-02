...
export default function StudentApp() {
  ...
  async function askDoubt() {
    ...
    try {
      ...
      parts.push({ text: `You are PID Institute's AI tutor. Student: ${student.studentName}, Class: ${student.class}. Answer in Hindi+English mix. Be detailed.\n\nQuestion: ${q || "Explain this image"}` });
  ...
  const C = { bg: "#0a0a1a", card: "#12122a", card2: "#1a1a36", text: "#eef0ff", text2: "#9ba3c7", text3: "#5c6490", border: "#252550", accent: "#635bff", accent2: "#8b83ff", green: "#00d4aa", red: "#ff6b6b", yellow: "#ffc53d", pink: "#ff6b9d", orange: "#ff9f43", cyan: "#00d2ff", gradMain: "linear-gradient(135deg, #635bff 0%, #ff6b9d 100%)", gradGreen: "linear-gradient(135deg, #00d4aa 0%, #00b4d8 100%)", gradOrange: "linear-gradient(135deg, #ff9f43 0%, #ff6b6b 100%)", gradCyan: "linear-gradient(135deg, #00d2ff 0%, #635bff 100%)" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', sans-serif", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      <div style={{ paddingBottom: 80 }}>
        {/* ═══ TOP BAR ═══ */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px 8px", position: "sticky", top: 0, background: "rgba(10,10,26,.92)", backdropFilter: "blur(20px)", zIndex: 50, borderBottom: `1px solid ${C.border}` }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div onClick={() => nav("profile")} style={{ width: 38, height: 38, borderRadius: 12, background: C.gradMain, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 4px 15px rgba(99,91,255,.3)", overflow: "hidden" }}>
        ...
        {activeTab === "ai" && (
          <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 140px)" }}>
            <div style={{ display: "flex", gap: 6, padding: "8px 20px" }}>
              {[{ id: "doubt", l: "Doubt Solver", i: "robot" }, { id: "quiz", l: "AI Quiz", i: "brain" }, { id: "history", l: "History", i: "history" }].map(t => (
            ...
            {aiSubTab === "doubt" && (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "0 20px" }}>
                <div style={{ flex: 1, overflowY: "auto", padding: "10px 0" }}>
                  {dChat.length === 0 && <div style={{ textAlign: "center", padding: 40, color: C.text3 }}><div style={{ width: 60, height: 60, borderRadius: 20, background: C.card2, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px" }}><I n="robot" s={26} c={C.accent} /></div><p style={{ fontSize: ".88rem", fontWeight: 600 }}>Ask any doubt!</p><p style={{ fontSize: ".72rem", color: C.text3 }}>Photo bhi bhej sakte ho</p></div>}
                  {dChat.map((msg, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start", marginBottom: 10 }}>
                      <div style={{ maxWidth: "85%", padding: "12px 16px", borderRadius: msg.role === "user" ? "18px 18px 4px 18px" : "18px 18px 18px 4px", background: msg.role === "user" ? C.accent : C.card, color: msg.role === "user" ? "#fff" : C.text, fontSize: ".84rem", lineHeight: 1.6, border: msg.role === "ai" ? `1px solid ${C.border}` : "none" }}>
                        {msg.img && <img src={msg.img} alt="" style={{ maxWidth: "100%", borderRadius: 12, marginBottom: 8 }} />}
                        {msg.role === "ai" ? renderMd(msg.text) : msg.text}
                      </div>
                    </div>
                  ))}
                  {dLoading && <div style={{ padding: 10, textAlign: "center" }}><I n="spinner" s={18} c={C.accent} /></div>}
                  <div ref={chatEndRef} />
                </div>
                ...
                <div style={{ display: "flex", gap: 8, padding: "10px 0 16px" }}>
                  ...
                  <button onClick={() => fileInputRef.current?.click()} style={{ width: 42, height: 42, borderRadius: 14, border: `1px solid ${C.border}`, background: C.card, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><I n="camera" s={15} c={C.text3} /></button>
                  <input value={dInput} onChange={e => setDInput(e.target.value)} onKeyDown={e => e.key === "Enter" && askDoubt()} placeholder="Type your doubt..." style={{ flex: 1, padding: "10px 16px", borderRadius: 14, border: `1px solid ${C.border}`, background: C.card, color: C.text, fontSize: ".85rem", outline: "none" }} />
                  <button onClick={askDoubt} disabled={dLoading} style={{ width: 42, height: 42, borderRadius: 14, background: C.gradMain, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><I n="paper-plane" s={15} c="#fff" /></button>
                </div>
              </div>
            )}
            {aiSubTab === "quiz" && (
            ...
            {aiSubTab === "history" && (
              <div style={{ padding: "10px 20px", flex: 1, overflowY: "auto" }}>
                ...
                : quizHistory.filter(q => q.percentage !== undefined).slice(0, 10).map((q, i) => (
                  <div key={q.id || i} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 14px", marginBottom: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div><p style={{ margin: 0, fontSize: ".8rem", fontWeight: 700 }}>{q.subject} {q.chapter ? `· ${q.chapter}` : ""}</p><p style={{ margin: "2px 0 0", fontSize: ".62rem", color: C.text3 }}>{q.score}/{q.total} · {q.difficulty}</p></div>
                    <div style={{ fontSize: "1rem", fontWeight: 900, color: (q.percentage || 0) >= 70 ? C.green : C.yellow }}>{q.percentage}%</div>
                  </div>
                ))}
                {/* Doubt History */}
                ...
                : doubtHistory.slice(0, 15).map((d, i) => (
                  <div key={d.id || i} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 14px", marginBottom: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                      ...
                      {d.hasImage && <span style={{ fontSize: ".6rem", padding: "2px 6px", borderRadius: 4, background: C.card2, color: C.orange, fontWeight: 600 }}>Image</span>}
      ...
      {/* ═══ BOTTOM NAV ═══ */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480, background: "rgba(10,10,26,.95)", backdropFilter: "blur(20px)", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "space-around", padding: "8px 6px 20px", zIndex: 100 }}>
        {[{ id: "home", i: "home", l: "Home" }, { id: "batches", i: "book-open", l: "Study" }, { id: "ai", i: "robot", l: "AI" }, { id: "tests", i: "laptop-code", l: "Tests" }, { id: "attendance", i: "calendar-check", l: "Attend" }, { id: "performance", i: "chart-line", l: "Rank" }].map(t => (
          <button key={t.id} onClick={() => nav(t.id)} style={{ background: "none", border: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, color: activeTab === t.id ? C.accent2 : C.text3, cursor: "pointer", flex: 1, position: "relative" }}>
            ...
            <div style={{ width: 30, height: 30, borderRadius: 10, background: activeTab === t.id ? "rgba(99,91,255,.12)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}><I n={t.i} s={14} /></div>
