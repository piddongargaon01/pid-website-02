...
// ── Year-wise Featured Toppers Component ──
function FeaturedToppersSection({ featuredToppers }) {
  if (featuredToppers.length === 0) {
    return (
  ...
  // Year-wise group banao
  const yg = {};
  featuredToppers.forEach(function(t) {
    const yr = t.year || "Other";
    if (!yg[yr]) yg[yr] = [];
    yg[yr].push(t);
  });
  const sortedYrs = Object.keys(yg).sort(function(a, b) { return b.localeCompare(a); });

  return (
    <div>
      {sortedYrs.map(function(yr, yi) {
        return (
          <div key={yr} style={{marginBottom:32}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              ...
              <span style={{fontSize:".72rem",color:"#6B7F99",fontWeight:600,background:"#EFF6FF",padding:"3px 10px",borderRadius:99,border:"1px solid #BFDBFE"}}>
                {yg[yr].length} Topper{yg[yr].length > 1 ? "s" : ""}
              </span>
            </div>

            {/* Toppers Cards Grid */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:14}}>
              {yg[yr].map(function(t, i) {
                return (
                  <div
                    key={t.id}
                    ...
                    onMouseEnter={function(e) {
                      ...
                      e.currentTarget.style.boxShadow = "0 14px 36px rgba(7,41,107,.12)";
                      e.currentTarget.style.borderColor = "#BFDBFE";
                    }}
                    onMouseLeave={function(e) {
                      e.currentTarget.style.transform = "none";
                      e.currentTarget.style.boxShadow = "none";
                      e.currentTarget.style.borderColor = "#D4DEF0";
                    }}
                  >
                    {/* Photo Box — Portrait size friendly */}
                    <div style={{height:230,background:"linear-gradient(160deg," + colors[i % 6] + "," + colors[(i + 2) % 6] + ")",position:"relative",overflow:"hidden"}}>
                      ...
                      {/* Percentage Badge */}
                      {t.percentage && (
                        <div style={{position:"absolute",top:10,right:10,background:"#F5AC10",color:"#fff",padding:"4px 11px",borderRadius:99,fontSize:".74rem",fontWeight:800,boxShadow:"0 2px 8px rgba(0,0,0,.25)"}}>
                          {t.percentage}
                        </div>
                      )}
                      {/* Rank overlay */}
                    ...
                    <div style={{padding:"11px 13px 12px"}}>
                      <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:".88rem",color:"#0B1826",marginBottom:3,lineHeight:1.3}}>
                        {t.name}
                      </div>
                      <div style={{fontSize:".72rem",color:"#4A5E78",display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                        {t.class && (
                          <span style={{background:"#EFF6FF",color:"#1349A8",padding:"1px 8px",borderRadius:99,fontWeight:600}}>
                            {t.class}
                          </span>
                        )}
...
export default function ToppersPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "featured_toppers"), (snap) => {
      const arr = snap.docs.map(function(d) { return { id: d.id, ...d.data() }; });
      arr.sort(function(a, b) { return (b.year || "").localeCompare(a.year || ""); });
    ...
    <>
      ...
      <div style={{background:"linear-gradient(135deg,#062560,#1548B0)",paddingTop:60,paddingBottom:40}}>
        <div className="wrap">
          ...
          <h1 style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:"clamp(1.6rem,3vw,2.2rem)",fontWeight:800,color:"#fff",marginBottom:6}}>
            Our Toppers — Year Wise
          </h1>
          <p style={{color:"rgba(255,255,255,.7)",fontSize:".88rem",maxWidth:560}}>
            Celebrating the achievements of our brilliant students who made us proud year after year. 99% pass rate since 2012.
          </p>
        </div>
      </div>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #E2EAF4",padding:"16px 0"}}>
        <div className="wrap" style={{display:"flex",justifyContent:"center",gap:32,flexWrap:"wrap"}}>
          {[{n:"99%",l:"Pass Rate"},{n:"50+",l:"Toppers"},{n:"5000+",l:"Students"},{n:"13+",l:"Years"}].map(function(s, i) {
            return (
      ...
      <div style={{background:"#F0F4FA",minHeight:"60vh"}}>
        <div className="wrap" style={{padding:"32px 20px"}}>
          ...
          {/* ═══ FEATURED TOPPERS — Year Batch wise ═══ */}
          <div style={{marginBottom:36}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              ...
              <div>
                ...
                <p style={{fontSize:".74rem",color:"#6B7F99"}}>Batch-wise star performers with their achievements</p>
              </div>
            </div>

            <FeaturedToppersSection featuredToppers={featuredToppers} />
          </div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
            {categories.map(function(cat) {
              return (
                <button
                  key={cat}
                  onClick={function() { setActiveTab(cat); }}
                  style={{
                    padding:"8px 16px",borderRadius:99,
                    border: activeTab === cat ? "2px solid #1349A8" : "1px solid #D4DEF0",
                    background: activeTab === cat ? "#1349A8" : "#fff",
                    color: activeTab === cat ? "#fff" : "#4A5E78",
                    fontSize:".74rem",fontWeight:600,cursor:"pointer",transition:"all .2s",
          ...
          <div style={{background:"#fff",borderRadius:14,border:"1px solid #D4DEF0",overflow:"hidden",marginBottom:20}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:".82rem"}}>
              ...
              <tbody>
                {data.toppers.map(function(t, i) {
                  return (
                    <tr
                      key={i}
                      style={{borderBottom:"1px solid #E8EFF8",transition:"background .15s"}}
                      onMouseEnter={function(e) { e.currentTarget.style.background = "#F8FAFD"; }}
                    ...
                    >
                      ...
                      <td style={{padding:"12px 16px"}}>
                        {t.pct ? (
                          <span style={{background:"#FEF3C7",color:"#92400E",padding:"3px 10px",borderRadius:99,fontSize:".74rem",fontWeight:700}}>{t.pct}</span>
                        ) : (
                      ...
                      <td style={{padding:"12px 16px"}}>
                        {t.rank ? (
                          <span style={{background:"#EFF6FF",color:"#1349A8",padding:"3px 10px",borderRadius:99,fontSize:".72rem",fontWeight:600}}>{t.rank}</span>
                        ) : (
                          <span style={{color:"#B0C4DC"}}>—</span>
                        )}
                      </td>
          ...
          <div style={{background:"#FFFBEB",borderRadius:12,padding:16,border:"1px solid #FDE68A",fontSize:".8rem",color:"#78350F",display:"flex",alignItems:"flex-start",gap:10,marginBottom:24}}>
            ...
            <div>
              <strong>Note:</strong> Some years are marked as gap years due to COVID-19 pandemic. Topper photos and additional details can be managed by admin from the Admin Panel.
            </div>
          </div>

          {/* CTA */}
          <div style={{textAlign:"center"}}>
            <Link href="/#enquiry" className="btn ba">
              Join PID &amp; Become a Topper <i className="fas fa-arrow-right"/>
            </Link>
          </div>
      ...
      {/* WhatsApp */}
      <a href="https://wa.me/918319002877" target="_blank" rel="noopener noreferrer" className="wa-float">
